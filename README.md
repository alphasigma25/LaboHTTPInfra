...
Dans cette étape, on modifie les fichiers suivants dans docker-images/apache-reverse-proxy :
- Dockerfile : on rajoute deux modes pour permettre le load-balancing
  ```dockerfile
  COPY templates /var/apache2/templates
  COPY conf/ /etc/apache2
  
  # rajout du mode proxy_balancer et lbmethod_byrequests
  RUN a2enmod proxy proxy_http proxy_balancer lbmethod_byrequests
  RUN a2ensite 000-* 001-*
  ```
- apache2-foreground : on double les lignes pour 
  ```bash
  echo "Static app1 URL: $STATIC_APP1"
  echo "Dynamic app1 URL: $DYNAMIC_APP1"
  echo "Static app2 URL: $STATIC_APP2"
  echo "Dynamic app2 URL: $DYNAMIC_APP2"
  ```
...
Pour démarrer tous les dockers dans le bon ordre, un fichier `start_docker.bat` a été rajouté à la racine du projet :
```bash
#build
#reverse proxy
...
Pour que tout marche, il faut s'assurer, comme pour les étapes précédentes que les adresses des différents dockers créés correspondent, et qu'il n'y ait pas des conteneurs avec des noms homonymes déjà existants lorsqu'on lance les commandes.
